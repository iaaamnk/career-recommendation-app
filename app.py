...
def get_top_alternatives(probas, predicted_index, top_n=3):
    ...
    for idx in sorted_indices:
        if idx != predicted_index and count < top_n:
            careers = target_le.inverse_transform([idx])
            career = str(careers[0])
...
def load_and_train_model():
    global rf_model, skill_mlb, interest_mlb, target_le
...
def predict():
    ...
    top_3_careers = [{"career": carrier_name, "score": round(float(carrier_score), 4)} for carrier_name, carrier_score in top_alternatives]
    
    riasec_dict = dict(zip(riasec_cols, riasec))
    top_riasec = sorted(riasec_dict.items(), key=lambda item: item[1], reverse=True)[:2]
    
    explanation = (
        f"The system recommends **{recommended_career}** with a {confidence*100:.1f}% match probability. "
        f"Your profile shows strong alignment with **{top_riasec[0][0]}** and **{top_riasec[1][0]}** traits."
    )
