...
class ResumeAnalyzerService:
    def __init__(self):
        self.careers = list(CAREER_SKILL_CORPUS.keys())
        corpus = [CAREER_SKILL_CORPUS[c] for c in self.careers]
        ...
        self.career_tfidf_matrix = self.vectorizer.fit_transform(corpus)

    def _match_career_key(self, target_career: str, mapping: dict) -> str:
    ...
    def analyze_resume_text(self, resume_text: str, target_career: str):
        text_lower = resume_text.lower()
        target_key = self._match_career_key(target_career, CAREER_SKILL_CORPUS)
        target_idx = self.careers.index(target_key)

        resume_tfidf = self.vectorizer.transform([text_lower])
        similarity = cosine_similarity(resume_tfidf, self.career_tfidf_matrix[target_idx])[0][0]

        all_similarities = cosine_similarity(resume_tfidf, self.career_tfidf_matrix)[0]
        top_career_indices = np.argsort(all_similarities)[::-1][:3]
        top_matching_careers = [
            {"career": self.careers[i], "similarity": round(float(all_similarities[i]) * 100, 1)}
            for i in top_career_indices
        ]

        ats_score = min(100.0, round(similarity * 150, 2))

        skill_key = self._match_career_key(target_career, CAREER_SKILLS_LIST)
    ...
    def generate_interview_prep(self, target_career: str, missing_skills: list):
        ...
        tech_key = self._match_career_key(target_career, _TECHNICAL_TEMPLATES)
...
nlp_service = ResumeAnalyzerService()
